...
новой схеме — а его INSERT без `origin` теперь падает. Откат на образ до #298
поэтому идёт мимо `deploy.sh`: шаг миграций исполняется ТЕМ образом, который
катят, и старый образ встаёт на `Can't locate revision identified by '0025'` —
штатный откат не состоится вовсе. Рецепт: вернуть колонке умолчание
(`ALTER COLUMN origin SET DEFAULT 'guest_chat'`), поднять старый образ вручную,
а при возврате вперёд умолчание снять; по шагам — `docs/runbooks/deploy.md`,
часть C. Шага назад (`alembic downgrade 0024`) в рецепте нет намеренно: он
сносит все пять колонок, `claimed_at` не восстанавливается ничем, а следующий
деплой прогонит эту миграцию заново и проставит `origin = 'guest_chat'` ВСЕМ
строкам разом — включая заведённые с `origin='api'` или `'staff_manual'`, то
есть сдвинет измеряемую долю и не скажет об этом.

Риск принят осознанно: снять `server_default` дешевле всего именно сейчас —
пилот не начался, живых гостей нет, окно деплоя стоит ноль. Отложить снятие до
следующего PR серии значит платить ту же цену на работающем отеле, где в окне
деплоя уже есть гости. Соседняя 0024 выбрала обратное и по другой причине:
у `is_urgent` умолчание `false` — честный ответ для любой вставки, у `origin`
честного умолчания не существует.
"""
