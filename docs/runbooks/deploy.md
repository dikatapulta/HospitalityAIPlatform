...
**Команда выше работает, только пока откат не пересекает миграцию.** Шаг миграций
гоняется **тем образом, который катят**: старый образ не знает ревизию, до которой
уже поднята БД, и встаёт с `Can't locate revision identified by '<ревизия>'`. К
обратной совместимости это отношения не имеет — так же встаёт откат через
безобидную `0024`. До `up -d` стека деплой при этом не доходит, на сервере
продолжает работать новая версия. Выход один на все такие случаи: поднять старый
образ мимо `deploy.sh` — это шаг 2 рецепта «Откат на образ старше #298» ниже, и
от того, какая миграция мешает, он не зависит.

**Схему откат не опускает** — шага назад у `deploy.sh` нет вовсе. Обычно это
безразлично: миграции обратно-совместимы в рамках одного деплоя (см. комментарий
в самом скрипте), и старый код живёт на новой схеме. Исключения — в таблице ниже:
на них старому коду новая схема мешает, и к обходу `deploy.sh` добавляется работа
со схемой — шаги 1 и 3 того же рецепта.

| Откат на образ старше | Что делать |
|---|---|
| #298 (миграция `0025`, источник заявки) | Старый код не заполняет `service_requests.origin`: колонка `NOT NULL`, умолчания у неё больше нет — создание заявок упало бы целиком, гость получал бы ошибку на каждое обращение. Рецепт — «Откат на образ старше #298» сразу ниже |

### Откат на образ старше #298

Схему при этом **не опускаем**: `alembic downgrade 0024` сносит все пять колонок
измеримости, `claimed_at` не восстанавливается ничем, а следующий деплой прогонит
`0025` заново и проставит `origin = 'guest_chat'` всем строкам разом — включая
заведённые с `origin=api` или `staff_manual`. Условия по времени в миграции нет,
поэтому недостоверной становится **вся история**, а не окно отката, и в
благоприятную сторону, то есть молча (проверено прогоном на временной БД: три
строки из трёх, включая заведённые за месяц до отката). Само окно отката неполно
и по «хорошему» пути ниже: пока работает старый образ, `claimed_at` и `closed_at`
не пишутся вовсе — он о колонках не знает. Разница в том, что там теряется только
окно, а здесь — вся история.
Поэтому вместо `downgrade` — вернуть колонке умолчание:

1. Открыть psql (канон входа — [subject-requests.md](subject-requests.md), §2;
   `sh -c` в одинарных кавычках обязателен: `POSTGRES_*` живут внутри контейнера):
   ```bash
   ssh deploy@<IP>
   cd /opt/hospitality
   docker compose -f docker-compose.staging.yml --env-file .env \
   ...
   ```
   ```sql
   ALTER TABLE service_requests ALTER COLUMN origin SET DEFAULT 'guest_chat';
   ```
   Старый код после этого снова создаёт заявки (все они получат `guest_chat`), а
   пять колонок остаются на месте вместе с собранными данными.
2. Поднять старый образ мимо `deploy.sh` — переменная в командной строке
   перебивает `APP_IMAGE` из `.env`:
   ```bash
   APP_IMAGE=ghcr.io/<owner>/hospitality-app:<старый-sha> \
     docker compose -f docker-compose.staging.yml --env-file .env up -d --wait
   ```
   Путь вперёд после починки — `./deploy.sh <sha образа с #298>`, аргументом.
   Без аргумента можно только если `.env` уже называет образ с #298: `deploy.sh`
   пишет туда образ лишь после зелёного smoke ([restore.md](restore.md), случай В,
   шаг 6), а красный smoke — самый частый повод откатываться, и тогда в `.env`
   стоит образ **до** #298. Голый `./deploy.sh` покатит тогда его же и встанет на
   `Can't locate revision identified by '0025'` — то есть вернёт в отказ,
   с которого этот рецепт начался.
3. Вернувшись на образ с #298, снять умолчание обратно — тем же psql:
   ```sql
   ALTER TABLE service_requests ALTER COLUMN origin DROP DEFAULT;
   ```
   Само оно не снимется: ревизия в БД так и осталась `0025`, поэтому
   `alembic upgrade head` при возврате — no-op, и миграция не исполняется. Забыть
   этот шаг — молча снять второй щит обязательности (spec 0035 §4): БД перестаёт
   ловить INSERT **мимо** `ServiceRequestCreate` — миграцию данных, ручной SQL,
   будущий путь создания, — и такая строка молча уедет в измеряемую долю как
   `guest_chat`. Первый щит (Pydantic и mypy на пути через код) остаётся.
...
| Откат `./deploy.sh <старый-sha>` встал, в логе `Can't locate revision identified by '<ревизия>'` (последний `==>` перед этим — «Снимок БД перед миграцией») | Откат **не состоялся** (до `up -d` стека деплой не дошёл), работает новая версия: старый образ не знает ревизию, до которой уже поднята БД. Так встаёт **любой** откат через миграцию, а не только особый: поднимать старый образ надо мимо `deploy.sh` — часть C, шаг 2 рецепта |
| Заявки не создаются, в логах `null value in column "origin"` | На сервере образ старше #298, а умолчание колонке не возвращено — шаг 1 рецепта «Откат на образ старше #298» (часть C) |
