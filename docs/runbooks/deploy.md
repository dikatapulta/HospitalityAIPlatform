...
откатывалась бы из ночного дампа с потерей до суток данных. Откат по снимку —
[restore.md](restore.md), случай В.
...
Одно исключение из «каждой команды» записано в самом compose: сервису `alerter`
`LLM_MODEL` не пробрасывается намеренно — watchdog обязан пережить негодную
конфигурацию приложения, иначе некому будет о ней сообщить.
