...
и на них команда выше не работает вовсе: шаг миграций гоняется **тем образом,
который катят**, а старый образ не знает новую ревизию и встаёт с
`Can't locate revision identified by '<ревизия>'` — до `up -d` деплой не доходит,
на сервере продолжает работать новая версия. Откат такого образа делается мимо
`deploy.sh`, по рецепту из последней колонки.
...
заведённые с `origin=api` или `staff_manual`. Измерения за период отката после
этого недостоверны, причём в благоприятную сторону, то есть молча (проверено
прогоном на временной БД). Вместо этого — вернуть колонке умолчание:
...
2. Поднять старый образ мимо `deploy.sh` — переменная в командной строке
   ...
   ```
   `.env` при этом продолжает называть новый образ, и это правильно: путь вперёд —
   обычный `./deploy.sh` (без аргумента) после починки.
3. Вернувшись на образ с #298, снять умолчание обратно — тем же psql:
   ...
   этот шаг — молча вернуть дыру, которую закрывает spec 0035 §4: путь, не
   назвавший источник, снова пишет `guest_chat` прямо в измеряемую долю.
...
| Откат `./deploy.sh <старый-sha>` встал на шаге «Применяю миграции БД», в логе `Can't locate revision identified by '<ревизия>'` | Откат **не состоялся** (до `up -d` деплой не дошёл), работает новая версия: старый образ не знает ревизию, до которой уже поднята БД. Часть C, таблица исключений |
