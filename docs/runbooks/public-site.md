...
файлы из GitHub и раздаёт их с серверов Cloudflare. **Начинать — только когда папка
`site/` уже есть в `main`:** Pages собирает сайт из `main`, и без неё первая выкладка
упадёт с ошибкой «нет каталога».
