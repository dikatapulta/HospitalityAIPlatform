...
- **Кто заметит, что бэкапы кончились** (issue #106): алертер. Каталог бэкапов
  смонтирован в его контейнер только для чтения, и если свежайшего
  `hospitality-*.dump.age` нет больше `ALERT_BACKUP_MAX_AGE_HOURS` (30 часов) —
  в Telegram-канал команды приходит ERR-OPS-008
  ([alerts.md](alerts.md#err-ops-008)). До этого единственными сигналами были
  еженедельный `make backup-fetch` и чтение `backup.log` руками, то есть
  фактически ничего: fail-closed шифрование делает «бэкапа нет вовсе» штатным
  и совершенно бесшумным исходом.
...
`OK: бэкап создан, проверен и зашифрован`. Ходить руками нужно только при
разборе алерта: с issue #106 остановившийся бэкап сам присылает ERR-OPS-008.
...
Команды шагов 1–4 и 6 — на сервере (`ssh deploy@<IP>`, `cd /opt/hospitality`);
шаг 5 (расшифровка + восстановление) и приёмка — **с машины основателя**:
приватный ключ есть только там.
...
1. **Понять, надо ли вообще откатывать.** Миграция часто падает, ничего не
   ...
   Сама команда упала на конфигурации, не дойдя до БД, — это тот же ответ:
   схема не тронута, чинить `.env` ([deploy.md](deploy.md), «Диагностика»).
...
6. **Поднять последнюю здоровую версию.** `deploy.sh` переписывает `APP_IMAGE`
   ...
   ```
   Запрета из шага 5 это не нарушает: `alembic upgrade head` внутри `deploy.sh`
   гоняется **старым** образом, а его head и есть схема снимка — команда ничего
   не изменит. Свой снимок перед этой миграцией деплой снимет заново, так и надо.
