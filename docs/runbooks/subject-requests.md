...
4. Обращение пришло на privacy@necturn.com — отвечать с этого же адреса.
   Закрыв обращение, удалить переписку из ящика вместе с «Корзиной»: политика
   (§2) обещает, что письмо живёт только до закрытия; в журнале остаётся
   строка из п. 3.
