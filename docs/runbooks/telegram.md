...
## Шаг 2б. Чаты отдельных служб (spec 0026, issue #80) — по желанию

По умолчанию **все** уведомления идут в один `TELEGRAM_STAFF_CHAT_ID`. Чтобы
уборка, инженерия и IT читали каждый свою ленту, заведите им отдельные группы
(шаг 2а повторяется для каждой) и свяжите категорию заявки с чатом. Это **конфиг
тенанта**, а не `.env`: разным отелям — разные службы.

```bash
# посмотреть текущий маппинг
docker compose -f /opt/hospitality/docker-compose.staging.yml exec app \
  python -m hospitality.tools.staff_routing

# задать: категория=chat_id (перечислять ЦЕЛИКОМ — команда заменяет весь маппинг)
docker compose -f /opt/hospitality/docker-compose.staging.yml exec app \
  python -m hospitality.tools.staff_routing housekeeping=-1001111 maintenance=-1002222

# вернуть всё в общий чат
docker compose -f /opt/hospitality/docker-compose.staging.yml exec app \
  python -m hospitality.tools.staff_routing --clear
```

Ключи категорий — те, что засеяны `make seed`: `housekeeping`, `maintenance`,
`it-support`. Неизвестный ключ команда не примет (покажет доступные): опечатка
выглядела бы рабочей настройкой, а уведомления молча шли бы в общий чат.

Что важно знать:

- **Бот должен быть в каждой такой группе** — иначе отправка упадёт, а воркер
  будет ретраить уведомление до исчерпания попыток.
- Категория без маппинга — в общий чат; эскалации («🆘 Гостю нужен сотрудник»)
  всегда в общий чат.
- **Каждый чат из маппинга становится staff-чатом**: в нём работают команды
  `/start · /done · /cancel`, и любой участник группы может двигать заявки
