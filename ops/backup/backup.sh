...
mkdir -p "$BACKUP_DIR"
# В каталог обязан войти ЧУЖОЙ uid: алертер читает его из контейнера (под
# uid 10001, issue #106) и по датам файлов видит, что ночной бэкап перестал
# появляться. Права самих дампов это не трогает — их держит umask 077 (0600),
# а сами они зашифрованы; наружу видны только имена и даты файлов. Явный chmod,
# а не расчёт на umask: `mkdir -p` про существующий каталог молчит, и на
# серверах, где каталог создан до issue #106, он остался бы 0700.
if ! chmod o+rx "$BACKUP_DIR"; then
    echo "ВНИМАНИЕ: не удалось открыть ${BACKUP_DIR} на вход чужому uid." >&2
    echo "Бэкап это не останавливает, но алертер пришлёт ERR-OPS-008 «каталог" >&2
    echo "бэкапов не виден» — как чинить, docs/runbooks/alerts.md#err-ops-008." >&2
fi
