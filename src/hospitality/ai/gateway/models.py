...
class LlmBudgetReservation(Base):
    ...
    следующем резерве того же тенанта. Оценка `amount_usd` пессимистична
    (полный `max_tokens` на выходе) — резерв обязан не занижать вызов, иначе
    он не ограничивает то, ради чего заведён.
    """
