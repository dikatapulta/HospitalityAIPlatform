...
# пессимистично для латиницы (~4) и близко к правде для кириллицы и казахского,
# на которых токенов на тот же текст выходит заметно больше (ADR-010). Резерв
# обязан НЕ занижать вызов: заниженный не ограничивает то, ради чего заведён.
...
def _reserve_usd(request: LlmRequest) -> Decimal:
    ...
    вход — оценкой по длине сериализованного запроса. Цена — из прайс-листа
    для `LLM_MODEL`; провайдер, собранный мимо настроек (bake-off, §7.7),
    получает самую дорогую строку прайс-листа, потому что резерв обязан быть
    пессимистичным, а `_compute_cost` с его `ValueError` здесь неуместен:
    исход вызова ещё неизвестен, ронять ход гостя не на чем.
    """
