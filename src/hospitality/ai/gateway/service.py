...
# каждого вызова обязана считаться (§7.2). Прежний комментарий здесь обещал
# падение «до обращения к провайдеру», хотя проверка стояла ПОСЛЕ ответа.
...
def validate_configured_model() -> None:
    ...
    ключи прайс-листа — id без даты, и матчится именно они.
