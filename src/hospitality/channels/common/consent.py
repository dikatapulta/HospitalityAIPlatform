...
_TEXTS = {
    "kk": (
        "Жалғастыра отырып, сіз дербес деректеріңіздің Құпиялылық саясатына "
        "сәйкес өңделуіне келісесіз: [саясат сілтемесі]"
    ),
    "ru": (
    ...
    "en": (
        "By continuing, you agree to the processing of personal data under the "
...
def consent_text(language: str | None) -> str:
    """Текст экрана согласия для показа гостю (spec 0029 §3, изменение v2/v3).

    С v3 (решение основателя 29.07) экран — одна строка согласия + ссылка на
    политику. Обязательные элементы (трансграничная передача — ст. 16, факт
    ИИ-обработки и право на сотрудника — ст. 19-1) раскрыты в политике по
    ссылке; достаточность такой формы — на проверке юриста
    (FOUNDER_REVIEW_QUEUE). Каналы показывают текст целиком, не сокращая
    (docs/legal/consent-text.md).
    """
