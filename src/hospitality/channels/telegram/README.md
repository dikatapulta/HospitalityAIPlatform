...
5. **Развилка гость / персонал** (`service.py`): чат == `TELEGRAM_STAFF_CHAT_ID` →
   ...
   - **Персонал** (`staff.py`): бот реагирует **только на команды** (текст с
     ведущим «/») — `/start · /done · /cancel <#N>` (ADR-013: `/assign` упразднён,
     бот отвечает подсказкой «сразу /start») → `requests.change_request_status`,
     ответ персоналу. Аргумент — **дневной
...
| `staff.py` | Команды персонала `/start · /done · /cancel <#N>` (Task 0017, ADR-013) |
