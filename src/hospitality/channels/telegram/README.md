...
5. **Развилка гость / персонал** (`service.py`): чат == `TELEGRAM_STAFF_CHAT_ID` →
   ...
   - **Персонал** (`staff.py`): бот реагирует **только на команды** (текст с
     ведущим «/») — `/assign · /start · /done · /cancel <id>` →
     `requests.change_request_status`, ответ персоналу. Обычная переписка группы и
     не-текст остаются без ответа (иначе бот спамит живую группу — её мьютят вместе
     с уведомлениями, #38 п.4).
6. **Уведомления** (`notifications.py`, подписчики событий, P-6): `request.created`
   → в staff-чат (категория + **номер комнаты** + **суть по-русски** через `ai.translation`
   + оригинал строкой «Гость написал» — персонал читает русский, оригинал рядом
   как эталон, баг #71; комнату подписчик дочитывает из заявки, событие её не несёт, #37);
   `request.status_changed(done)` → подтверждение гостю (адрес — по `request_origins`).
   Регистрируются composition root воркера (`hospitality/worker.py`).
...
| `notifications.py` | Подписчики событий: уведомление службе (суть переводит `ai.translation` на язык персонала), подтверждение гостю (P-6) |
