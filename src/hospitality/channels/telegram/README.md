...
5. **Развилка гость / персонал** (`service.py`): чат == `TELEGRAM_STAFF_CHAT_ID` →
   ...
   - **Персонал** (`staff.py`): `/assign · /start · /done · /cancel <id>` →
     `requests.change_request_status`, ответ персоналу.
6. **Уведомления** (`notifications.py`, подписчики событий, P-6): `request.created`
   → в staff-чат (категория + комната + **суть по-русски** через `ai.translation`
   + оригинал строкой «Гость написал» — персонал читает русский, оригинал рядом
   как эталон, баг #71); `request.status_changed(done)` → подтверждение гостю
   (адрес — по `request_origins`). Регистрируются composition root воркера (`hospitality/worker.py`).
