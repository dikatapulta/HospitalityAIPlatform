...
6. **Уведомления** (`notifications.py`, подписчики событий, P-6): `request.created`
   → в staff-чат (категория, **номер комнаты**, суть, id — комнату подписчик
   дочитывает из заявки, событие её не несёт, #37); `request.status_changed(done)`
   → подтверждение гостю (адрес — по `request_origins`). Регистрируются composition
