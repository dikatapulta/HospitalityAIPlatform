...
5. **Развилка гость / персонал** (`service.py`): чат == `TELEGRAM_STAFF_CHAT_ID` →
   ...
   - **Гость** (`guest.py`): не-текст → вежливый отказ; текст → **rate-limit
     ...
     дальше — оркестратор (история + `pending_action`), ответ гостю. Заявка
...
6. **Уведомления** (`notifications.py`, подписчики событий, P-6): `request.created`
   ...
   сбой перевода → канонический текст); `conversation.escalated` → «🆘 Гостю нужен
...
| `guest.py` | Гостевой ход: rate-limit до оркестратора (spec 0023), вызов оркестратора, история, `pending_action`, привязка заявки, публикация эскалации (spec 0022) |
...
уведомление службе; `request.status_changed` → подтверждение гостю при `done`;
`conversation.escalated` → эскалация в staff-чат (своё же событие: публикация —
вебхук-процесс, доставка — воркер).
