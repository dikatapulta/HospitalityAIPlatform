...
Публикует гостевой обработчик канала (`guest.py`) на обоих путях «зову
сотрудника»: исход `NEEDS_HUMAN` оркестратора и деградация §7.8 (LLM недоступен).
Потребляет подписчик `notifications.notify_staff_on_conversation_escalated`
(регистрирует `notifications.register()`, как остальные уведомления). Событие
композиционного слоя: доменные модули его не знают, шина (`shared/events`) —
kernel, слои не нарушены (развилка — spec 0022).
