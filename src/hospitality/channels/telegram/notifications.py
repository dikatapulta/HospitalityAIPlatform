...
async def notify_staff_on_request_created(
    ...
    *,
    ...
    перевода, идея основателя) — обе половины только при `origin == guest_chat`
    (spec 0035 §5; заявку, заведённую персоналом, переводить не с чего). Категория
    (из конфига тенанта, уже по-русски) и номер комнаты — явными строками: по ним
    персонал действует даже без сути.
    """
