...
последняя попытка, на которой `attempts >= WORKER_MAX_DELIVERY_ATTEMPTS`, приходится
по расписанию backoff уже за сроком годности, а подписчик там возвращается, а не
бросает — поэтому `dead_lettered_at` не выставляется никогда и `ERR-EVENTS-002` по
этому событию не приходит. Путь оживает, как только ПОСЛЕДНЯЯ по счёту попытка
попадает внутрь срока годности, а расписание задают три настройки воркера, не одна:
предел попыток, потолок backoff и его база. Пороги всех трёх — у владельца факта
(`docs/runbooks/errors.md`, ERR-TELEGRAM-006), там же и разбор; трогали любую —
считать расписание заново по формуле, а не по памяти.
