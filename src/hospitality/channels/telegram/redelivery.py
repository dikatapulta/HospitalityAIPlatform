...
оживёт, если предел попыток опустить до 9 и ниже: девятая попытка (510-я секунда)
— последняя внутри срока годности.
...
async def redeliver_reply(event: TelegramReplyUndelivered, *, sender: TelegramSender) -> None:
    ...
    нём: реплика, ушедшая гостю на прошлой попытке, но не дожившая до записи
    (смерть процесса между `send_message` и `record_outbound_message`),
    возвращается на доставку по истечении аренды — и при обратном порядке
    получила бы ERROR «ответ потерян окончательно», после чего рунбук послал бы
    человека отправить гостю то же самое второй раз.
    """
