"""Дожим ответа гостю, не ушедшего с первой попытки (issue #209, spec 0016 §P-8).
...
`WORKER_RETRY_BACKOFF_BASE_SECONDS` (2 с), дальше экспонента (ADR-009), исчерпав
попытки — dead-letter с алертом человеку (ADR-015). Сеть при этом лежит вне
транзакции (ADR-016), как у всякой доставки outbox.

**Протухание.** У гостевой реплики, в отличие от уведомления, есть срок годности:
она привязана к ходу диалога, и «принял, полотенце несут» через полчаса дезориентирует
...
никогда: ни при протухании, ни при dead-letter.
"""
...
# Код каталога ошибок (docs/runbooks/errors.md, R-8): ответ гостю потерян
# ОКОНЧАТЕЛЬНО — либо не встал в очередь, либо протух в ней.
ERR_TELEGRAM_REPLY_LOST = "ERR-TELEGRAM-006"

# Срок годности реплики. Дожим внутри него полезен (гость ещё ждёт ответа на своё
# сообщение), позже — вреден: разговор ушёл вперёд, а старая реплика приходит в
# него как чужая. 10 минут — окно, за которое backoff ADR-009 успевает 8 попыток
# (2+4+8+16+32+64+128+256 с), то есть переживает и 429, и обычный сбой Bot API;
# что дольше десяти минут — уже не сбой доставки, а недоступность канала, и о ней
# человеку говорит ERR-TELEGRAM-006, а не запоздалое сообщение гостю.
REPLY_MAX_AGE_SECONDS = 600.0


class GuestReplyUndelivered(DomainEvent):
    ...
    event_name: ClassVar[str] = "guest_reply.undelivered"
...
async def queue_undelivered_reply(
    ...
    *,
    ...
    event = GuestReplyUndelivered(
        ...
        # ключ проглотил бы вторую как дубль первой.
        idempotency_key=idempotency_key or f"guest:reply:{uuid.uuid4()}",
        queued_at=utc_now(),
    )
    try:
...
async def redeliver_guest_reply(event: GuestReplyUndelivered, *, sender: TelegramSender) -> None:
    """Подписчик `guest_reply.undelivered`: дожать реплику или объявить её потерянной.

    Сбой отправки ПРОБРАСЫВАЕТСЯ — это и есть ретрай: воркер отложит событие по
    backoff (ADR-009) и повторит, а исчерпав попытки — похоронит с алертом
    (ADR-015). Идемпотентность (P-8) — на естественном ключе реплики: повторная
    доставка события находит уже записанный исходящий `Message` и выходит.
    """
    age_seconds = (utc_now() - event.queued_at).total_seconds()
    if age_seconds > REPLY_MAX_AGE_SECONDS:
        # Возврат, а не исключение: событие помечается доставленным. Повторять
...
def register(*, sender: TelegramSender) -> None:
    ...
    async def on_guest_reply_undelivered(event: GuestReplyUndelivered) -> None:
        await redeliver_guest_reply(event, sender=sender)

    subscribe(GuestReplyUndelivered, on_guest_reply_undelivered)
