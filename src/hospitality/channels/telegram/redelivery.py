...
async def redeliver_reply(event: TelegramReplyUndelivered, *, sender: TelegramSender) -> None:
    ...
    `WORKER_DELIVERY_LEASE_SECONDS`, событие вернули в очередь руками по
    ADR-015) — а на поздних попытках это уже за десятиминутной границей. При
    обратном порядке такая реплика получила бы ERROR «ответ потерян
    окончательно», после чего рунбук послал бы человека отправить гостю то же
    самое второй раз.
