...
async def _resolve_target(
    ...
    # Аргумент цитируется персоналу — значит, проходит канон маскирования: ответ
    # не только уходит в группу, но и пишется строкой в `messages` (`send_reply`),
    # где лежит 90 дней. Карта через дефисы (`4111-1111-1111-1111`) — не цифры и
    # не uuid, поэтому доходит ровно сюда (ревью PR #202, NG-3, spec 0031 §2).
    # Разбор выше от этого не страдает: он идёт по сырому `explicit`.
    return (
