...
async def test_delivered_reply_is_not_declared_lost_after_the_deadline(
    ...
    """Реплика уже у гостя и записана, событие вернулось протухшим — не потеря (Н-1).

    Достижимо: строка outbox не получила `processed_at` — процесс умер между
    записью `Message` и фиксацией исхода либо истекла аренда
    `WORKER_DELIVERY_LEASE_SECONDS`; на поздних попытках возраст уже за
    десятиминутной границей. Сверив возраст ПЕРЕД фактом доставки,
    подписчик писал бы ERROR ERR-TELEGRAM-006 «ответ
    потерян окончательно» о реплике, которую гость получил, — а статья каталога
    велит оператору ответить руками, то есть отправить то же самое второй раз,
    теперь от лица человека.
    """
    configure_logging()
    conversation_id = await grant_consent(demo_tenant, GUEST_CHAT)
    sender = RecordingSender()
