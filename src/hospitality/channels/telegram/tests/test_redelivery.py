...
async def test_delivered_reply_is_not_declared_lost_after_the_deadline(
    ...
    `WORKER_DELIVERY_LEASE_SECONDS`, событие вернули в очередь руками (ADR-015);
    на поздних попытках возраст уже за десятиминутной границей. Сверив возраст
    ПЕРЕД фактом доставки, подписчик писал бы ERROR ERR-TELEGRAM-006 «ответ
