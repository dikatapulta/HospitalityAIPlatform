...
async def test_ensure_conversation_is_idempotent(demo_tenant: uuid.UUID) -> None:
    with tenant_context(demo_tenant):
        first = await ensure_conversation("telegram", "777")
        second = await ensure_conversation("telegram", "777")
    assert first == second


async def test_duplicate_delivery_key_returns_none(demo_tenant: uuid.UUID) -> None:
    with tenant_context(demo_tenant):
        conversation_id = await ensure_conversation("telegram", "777")
...
async def test_notification_already_sent(demo_tenant: uuid.UUID) -> None:
    ...
    with tenant_context(demo_tenant):
        conversation_id = await ensure_conversation("telegram", "staff-chat")
        assert await load_conversation_address(conversation_id) == ("telegram", "staff-chat")
