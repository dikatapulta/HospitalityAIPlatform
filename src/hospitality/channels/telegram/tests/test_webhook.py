...
БД. Отправитель ответов подменяется фейком через `dependency_overrides`.
"""
...
from hospitality.channels.telegram.router import ERR_TELEGRAM_BAD_SECRET, get_telegram_sender
from hospitality.channels.telegram.service import UNSUPPORTED_REPLY
...
async def test_text_message_stored_with_correlation_id(
    ...
    """DoD: сообщение появляется как Message в БД с correlation_id; ответ на текст
    в Task 0016 не шлётся (AI-ответ подключает Task 0017)."""
    client, sender, tenant_id = webhook
    response = await client.post(
    ...
    assert message.external_message_id == "7"
    # Прямая проверка DoD: строка связана со следом запроса в логах.
    assert message.correlation_id == response.headers["X-Correlation-ID"]
...
async def test_duplicate_update_creates_single_message(
    ...
    """Идемпотентность по update_id (P-8): повтор вебхука не создаёт второй Message."""
    client, _sender, tenant_id = webhook
