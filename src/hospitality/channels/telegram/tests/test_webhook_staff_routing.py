...
одной строкой, а членство в множестве, поэтому граница покрыта отдельно: чат
службы и чат утренней сводки становятся staff'ом, а гостевой чат не становится
им ни при каком конфиге (в том числе когда его id — подстрока id служебного
чата).
...
GUEST_CHAT = "555"
# Чат руководства отеля, куда уходит утренняя сводка (spec 0035 §8).
MANAGER_CHAT = "-1009999"
...
async def test_daily_summary_chat_is_treated_as_staff(
    stand: tuple[AsyncClient, RecordingSender, uuid.UUID],
) -> None:
    """Чат утренней сводки — персонал, а не гость (issue #301, ревью PR #329).

    Бота заводят в группу руководства, чтобы слать туда сводку, а Telegram в
    privacy mode доставляет ему реплаи на его собственные сообщения. Реплай
    менеджера на сводку — ожидаемый ход, и до этой строки он попадал в гостевую
    ветку: экран согласия гостя в группе руководства, а после нажатия кнопки —
    консьерж со своими инструментами и оплатой токенов за каждый ход.

    Согласие этому чату намеренно не выдаётся (в отличие от гостевых чатов
    фикстуры): считай его код гостевым — и первым же ответом придёт трёхъязычный
    экран согласия, а не подсказка по командам.
    """
    client, sender, tenant_id = stand
    await set_staff_routing(tenant_id, {}, daily_summary_chat_id=MANAGER_CHAT)

    await _say(client, MANAGER_CHAT, "/чепуха", 7)

    assert len(sender.sent) == 1
    chat_id, reply = sender.sent[0]
    assert chat_id == MANAGER_CHAT
    assert STAFF_HINT in reply
