...
- `list_requests_closed_since(closed_after=, limit=) -> list[ServiceRequestRead]`
  ...
  последняя запись в строку, поэтому `updated_at` — честный момент закрытия).
  Границу суток (полночь отеля) считает вызывающая сторона — модуль не знает
  про часовые пояса представления.
