...
Отдельный файл, а не ещё сотня строк в `service.py`: тот и без них за
границей R-3 «~400 строк». Вход снаружи — реэкспорт `day_summary` через `api.py`
...
class RequestsDaySummary(BaseModel):
    ...
    def has_no_requests(self) -> bool:
        """Ни одной заявки: ни создано, ни закрыто, ни открыто сейчас.

        Половина предиката «пустой день» (§9), та, что про числа этого модуля;
        вторую половину — «и бот никого не звал» — добавляет показывающая
        сторона, потому что эскалации считает другой владелец (§6). Считает её
        владелец чисел, а не каждая поверхность заново: пустой день короткой
        строкой показывают и страница кабинета, и утреннее сообщение (§8), а
        разойдясь, они назовут пустым разное.

        День, в который закрыли четыре вчерашние заявки, пустым не является,
        хотя создано в нём ноль.
        """
        return self.created_total == 0 and self.closed_total == 0 and self.open_now == 0
