...
Подмени пояс отеля на UTC — окно суток съедет, и держат этот съезд два теста
...
(сдвигается конец суток, на котором стоит вся отсечка просрочки). Оба стоят на
фиксированных датах прошлого и краснеют в любой час прогона. Остальные метки —
полдень отеля, он лежит и внутри суток UTC, поэтому подмену они обычно
переживают; но в часы, когда локальная дата отеля и дата UTC разошлись (для
Алматы, UTC+5 без перевода стрелок, — с 19:00 UTC), краснеют заодно и те, у кого
день сводки берётся из `hotel_today()`, а метки — от «сейчас». Имена двух щитов
от часа прогона не зависят, число покрасневших — зависит.
"""
