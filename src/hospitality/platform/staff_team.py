...
def _forbid_self_action(user_id: uuid.UUID, actor_user_id: uuid.UUID) -> None:
    ...
    Единственная защита от самоблокировки в v1: снять с себя роль `manager`
    или отключить свою учётку — потерять кабинет отеля без пути назад (сброса
    пароля и реактивации нет, ERR-AUTH-001/-005). Свой доступ закрывают через
