...
| --- | --- | --- |
| `config.py` | `get_settings()` — единственный способ читать конфигурацию окружения. Границу значения ставят на самом поле (`Literal` у `log_level`, `Field(ge=1)` у `llm_max_attempts`): негодная настройка обязана падать здесь, на сборке `Settings`, а не глубже в коде — процесс с ней не поднимается (issue #273; список fail-fast проверок старта — `hospitality/preflight.py`). Правилом всего модуля это пока не стало: границу несут два поля из четырёх десятков, остальные принимают 0 молча — **#283** | 0005, issue #273 |
