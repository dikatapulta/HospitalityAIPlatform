...
опирается на адрес СОКЕТА — его подделать нельзя, поэтому `--proxy-headers`
у uvicorn намеренно НЕ включён: он подменяет `scope["client"]` значением из
`X-Forwarded-For` (при `--forwarded-allow-ips=*` — САМЫМ ЛЕВЫМ, то есть
присланным клиентом), и проверка доверия стала бы проверять подделку.
