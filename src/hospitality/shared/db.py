...
async def platform_session_scope() -> AsyncIterator[AsyncSession]:
    ...
    Для работы с нетенантными таблицами: реестр тенантов (онбординг,
    сиды — Task 0011), служебная диагностика. Контекст тенанта не ставится,
    поэтому RLS-политики по умолчанию не пропустят ни чтение, ни запись
    тенантных таблиц. Единственное осознанное исключение — `outbox_events`
    (Task 0010, ADR-005, миграция 0003): её политика `platform_dispatch`
    разрешает платформенной сессии читать и обновлять очередь ВСЕХ тенантов —
    диспетчеру воркера это необходимо. Это исключение не копируется в
    бизнес-таблицы; для них «обойти изоляцию» через этот scope нельзя.
