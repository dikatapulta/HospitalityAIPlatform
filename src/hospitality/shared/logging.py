...
_SECRET_PATH_PATTERNS: Final = (
    ...
    re.compile(r"(/w/[^/]+/b/)[^/?#]+"),
    # Токен ссылки-приглашения сотрудника: /staff/invite/{token} (spec 0033 §3.4).
    # Живёт 72 часа и заводит учётку с ролью — секрет более ценный, чем гостевой.
    re.compile(r"(/staff/invite/)[^/?#]+"),
)
...
def redact_secrets_in_path(path: str) -> str:
    ...
    (`CorrelationIdMiddleware`) и `before_send` Sentry — там путь приходит
    сразу несколькими полями события (их перечень — в `sentry.py`). Путь без
    секретов возвращается как есть, поэтому вызов безопасно ставить на всякий
    путь.
    """
