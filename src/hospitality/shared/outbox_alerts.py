...
Сеть при этом не держит транзакцию: чтение, отправка и пометка — три отдельных
шага. Тем же порядком с issue #134 живёт и доставка событий (ADR-016).
...
async def alert_dead_letter_events(send: AlertSender, *, batch_size: int | None = None) -> int:
    ...
    рассказанными. Метка живёт до следующих похорон: `_record_outcome` снимает
    её, хороня событие снова, — вернувшееся в очередь и снова умершее событие
    рассказывается заново (ADR-015). Кросс-тенантное чтение outbox —
    платформенной сессией, как в `deliver_pending_events` (единственное
    осознанное исключение, миграция 0003).
