...
- HTTP-запросы — `TenantContextMiddleware` с резолвером по сервисному
  токену (`resolve_tenant_from_service_token`, platform/auth.py, Task 0013);
  запрос без валидного токена идёт дальше БЕЗ тенанта, и аутентифицированные
  эндпоинты отвечают 401 через `require_authenticated_tenant`;
...
# возвращает tenant_id или None. Асинхронный: реализация Phase 0 ищет тенанта
# в реестре по сервисному токену (`resolve_tenant_from_service_token`,
# platform/auth.py, Task 0013). Принимать tenant_id из произвольных клиентских
# заголовков запрещено (§11: клиент не выбирает себе тенанта).
TenantResolver = Callable[[Scope], Awaitable[uuid.UUID | None]]
...
class TenantContextMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        tenant_id = await self._resolver(scope)
        if tenant_id is None:
            await self._app(scope, receive, send)
            return
