...
| `static/styles.css` | CANONICAL: мобильный CSS-канон кабинета — палитра в `:root` (светлая и тёмная тема одними токенами), тач-таргеты ≥ 44px, отступы под вырез экрана (`env(safe-area-inset-*)`), классы `.card/.form/.btn/.list/.chips` (+чипсы-радио, `.access-code`, `.qr-box` — PR E; `.team-row`, `.invite-url` — PR F; `.badge-overdue`, `.request-card-overdue`, `.hint-alert` — просрочка); новые страницы переиспользуют, а не изобретают | #48 PR C/D/E/F |
