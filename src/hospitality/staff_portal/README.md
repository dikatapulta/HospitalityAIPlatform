...
| `checkin.py` | Данные и расчёты страницы заселения: разбор формы, «12:00 по поясу отеля → UTC» (заезд+ночи и продление), карточка Stay, URL и SVG-QR bind-ссылки и комнатного QR печатного листка (segno); отсюда же берут `hotel_zone`/`format_local` соседние страницы | #48 PR E |
...
| `static/checkin.js` | Ванильный JS карточки Stay: поллинг счётчика привязок каждые 3 с (индикатор «гость подключился»), отсчёт TTL QR, действия по CSRF-контракту, печать листка гостю (`window.print()`, код — с экрана); разметку не сочиняет — единственный innerHTML это ГОТОВЫЙ серверный SVG-QR из ответа | #48 PR E |
...
(серверный SVG-QR bind-ссылки и комнатного QR печатного листка).
...
- Новый ЦВЕТ в стилях → переменная в `:root` И её значение в блоке
  ...
  темой молча. Исключение одно — блок `@media print` печатного листка гостю
  (действует только на странице с листком: страничные селекторы — через
  `:has(.print-slip)`, остальные адресуют классы, которые есть только в
  карточке с листком):
  бумага, а не тема, и там чёрный по белому литералами намеренно. Белая
  подложка QR — не CSS, segno рисует её внутрь самой картинки
  (`checkin.qr_svg`, `light="#ffffff"`).
