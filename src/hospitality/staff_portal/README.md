...
(закрывает #56), PR E — страницу заселения (Stay, талон: QR ссылки привязки + код 6 цифр),
...
| `checkin.py` | Данные и расчёты страницы заселения: разбор формы, «12:00 по поясу отеля → UTC» (заезд+ночи и продление), карточка Stay, URL и SVG-QR ссылки привязки (segno); отсюда же берут `hotel_zone`/`format_local` соседние страницы | #48 PR E |
...
| `static/styles.css` | CANONICAL: мобильный CSS-канон кабинета — палитра в `:root` (светлая и тёмная тема одними токенами), тач-таргеты ≥ 44px, отступы под вырез экрана (`env(safe-area-inset-*)`), классы `.card/.form/.btn/.list/.chips` (+чипсы-радио, `.access-code`, `.qr-box` — PR E; `.print-only`/`.talon-*` и `@media print` — печать талона, #354; глобальное `[hidden]` — атрибут прячет и элементы с `display` из класса; `.team-row`, `.invite-url` — PR F; `.badge-overdue`, `.request-card-overdue`, `.hint-alert` — просрочка; `.badge-urgent`, `.request-card-urgent` — срочность, spec 0034; `textarea` — суть заявки в форме ручного приёма, #299; `.tiles`/`.tile`, `.data-table` — плитки и разрез по службам на сводке дня, #300); новые страницы переиспользуют, а не изобретают | #48 PR C/D/E/F, #299, #300 |
...
| `static/checkin.js` | Ванильный JS карточки Stay: поллинг счётчика привязок каждые 3 с (индикатор «гость подключился»), печать талона (`window.print`), действия по CSRF-контракту (перевыпуск возвращает код и QR разом); разметку не сочиняет — единственный innerHTML это ГОТОВЫЙ серверный SVG-QR из ответа | #48 PR E |
...
receptionist/manager; выпуск QR и перевыпуск талона под rate-limit `ERR-AUTH-010`),
...
- Новый ЦВЕТ в стилях → переменная в `:root` И её значение в блоке
  ...
  её внутрь самой картинки (`checkin.qr_svg`, `light="#ffffff"`). Печать —
  третий набор значений тех же токенов (`@media print` в конце файла): бумага
  светлая при любой теме телефона.
