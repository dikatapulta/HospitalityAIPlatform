...
| `summary.py` | Данные страницы «Сводка дня» (spec 0035 §6–§7): складывает числа их владельцев — `requests_api.day_summary` и `count_escalations` общего ядра каналов, — раскладывает по плиткам с надписями §9 и строит разрез «По службам»; окно суток для эскалаций берёт из ответа `day_summary`, а не считает второй раз. Надписи «29 августа» и «6 мин» — из `shared/humanize`: те же числа теми же словами печатает утреннее сообщение канала (#301), а импортировать кабинет ему запрещено (контракт 7) | #300, #301 |
