...
| `queue.py` | Данные страницы очереди: лента открытых / «закрытые за сегодня» (полночь отеля, фолбэк UTC), фильтры категория/«мои» (представление, в памяти), карточки и чипсы, метка «просрочена» по сроку тенанта (spec 0028) | #48 PR D |
...
| `static/styles.css` | CANONICAL: мобильный CSS-канон кабинета — палитра в `:root` (светлая и тёмная тема одними токенами), тач-таргеты ≥ 44px, отступы под вырез экрана (`env(safe-area-inset-*)`), классы `.card/.form/.btn/.list/.chips` (+чипсы-радио, `.access-code`, `.qr-box` — PR E; `.team-row`, `.invite-url` — PR F); новые страницы переиспользуют, а не изобретают | #48 PR C/D/E/F |
...
- Новый ЦВЕТ в стилях → переменная в `:root` И её значение в блоке
  ...
  темой молча. Единственное исключение в файле — белая подложка `.qr-box`:
  segno рисует QR жёстко чёрным (`stroke="#000"`), и без своей подложки код в
  тёмной теме не сканируется.
